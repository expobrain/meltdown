...
function inlineComponent(node, frame, options) {
    // Load component form symbols
    var componentName = node.openingElement.name.name,
        componentNode = frame.symbols[componentName];

    // Skip if replaceReactRouter is set and the component node is a
    // react-router component
    if (options.replaceReactRouter &&
            _.includes(componentName, constants.REACT_ROUTER_COMPONENTS)) {
        return;
    }
...
function findInlines(rootNode, frame, options) {
    utils.traverseTree(rootNode, function (node) {
        ...
        if (needInline) {
            inlineComponent(node, frame, options);
        }
    });
...
module.exports = function (frame, options) {
    return findInlines(frame.exports, frame, options);
};
