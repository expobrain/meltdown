...
function compileJSXElementLink(node, context) {
    ...
    to = _.chain(node.openingElement.attributes)
        .filter(function (attr) {
            return attr.type === 'JSXAttribute' && attr.name.name === 'to';
        })
        .first()
        .value();

    params = _.chain(node.openingElement.attributes)
        .filter(function (attr) {
            return attr.type === 'JSXAttribute' && attr.name.name === 'params';
        })
        .first()
        .value();

    context.output += '<a href="{% url ';
    context.output += "'" + to.value.value + "'";

    if (params) {
        _.forEach(params.value.expression.properties, function (prop) {
            context.output += ' ';
